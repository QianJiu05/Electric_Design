...
void spll_sogi_func(SOGI_PLL_DATA_DEF *spll_obj, float va, float vb, float vc)
{
    /* 1. Clarke 变换 -> α, β */
    float valpha = va;
    float vbeta  = (vb - vc) * ONE_BY_SQRT3;   // 1/√3 ≈ 0.57735
    /* 2. SOGI 产生正交信号 */
...
//float va = InputSignal(0,0);
//float vb = InputSignal(0,1);
//float vc = InputSignal(0,2);
//spll_sogi_func(&spll_data, va, vb, vc);
...
//float deta_theta = atan2(InputSignal(0,1), InputSignal(0,2));
//float temp_theta = spll_data.theta - deta_theta;
...
//    OutputSignal(0, 0) = temp_theta;
  //  OutputSignal(0, 1) = spll_data.pll_freq_out/value_2pi;
